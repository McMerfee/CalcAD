...
const ExtendableSystem = ({
  ...
  useTitle(t('routes.extendable-system'));
  ...
  useEffect(() => {
    if (_.isEmpty(orderID)) {
      dispatch(ConfigActions.getConfigRequest('extendable'));
      dispatch(SystemsActions.setCurrentSystem('extendable'));
    }
    if (_.isEmpty(orderID) && !isEditPage) {
      dispatch(DoorsActions.setDefaultsBySystemType('extendable'));
    }
  }, []);
  ...
  useEffect(() => {
    const handleLeavePage = (e) => {
      ...
      dispatch(NavigationActions.toggleLeavePageModal(true, '/saved-orders'));
  ...
  const tabsLabels = doors.map((door, i) => ({ title: t('extendableSystem.door-n', { number: i + 1 }) }));
  tabsLabels.unshift({ title: t('extendableSystem.basic') });
  ...
  return (
    <div className="extendable-system-page">
      <div className="sticky-container">
        ...
        {/* <ZoomVizualizationButton className="desktop" /> */}
...
ExtendableSystem.defaultProps = {
  orderID: null,
};

ExtendableSystem.propTypes = {
  toastManager: PropTypes.shape(notificationPropTypes).isRequired,
  orderID: PropTypes.string,
};

export default withToastManager(ExtendableSystem);
