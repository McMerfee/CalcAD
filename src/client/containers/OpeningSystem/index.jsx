...
const OpeningSystem = ({
  ...
  useTitle(t('routes.opening-system'));
  ...
  useEffect(() => {
    if (_.isEmpty(orderID)) {
      dispatch(ConfigActions.getConfigRequest('opening'));
      dispatch(SystemsActions.setCurrentSystem('opening'));
    }
    if (_.isEmpty(orderID) && !isEditPage) {
      dispatch(DoorsActions.setDefaultsBySystemType('opening'));
    }
  }, []);
  ...
  const tabsLabels = doors.map((door, i) => ({ title: t('openingSystem.door-n', { number: i + 1 }) }));
  tabsLabels.unshift({ title: t('openingSystem.basic') });
  ...
  if ((isLoading || !isConfigFetched) || (orderID && !isFetched)) {
    return (
      <div className="opening-system-page">
...
OpeningSystem.defaultProps = {
  orderID: null,
};

OpeningSystem.propTypes = {
  toastManager: PropTypes.shape(notificationPropTypes).isRequired,
  orderID: PropTypes.string,
};

export default withToastManager(OpeningSystem);
