...
const HingedSystem = ({
  ...
  useTitle(t('routes.hinged-system'));
  ...
  useEffect(() => {
    if (_.isEmpty(orderID)) {
      dispatch(ConfigActions.getConfigRequest('hinged'));
      dispatch(SystemsActions.setCurrentSystem('hinged'));
    }
    if (_.isEmpty(orderID) && !isEditPage) {
      dispatch(DoorsActions.setDefaultsBySystemType('hinged'));
    }
  }, []);
  ...
  const tabsLabels = doors.map((door, i) => ({ title: t('hingedSystem.door-n', { number: i + 1 }) }));
  tabsLabels.unshift({ title: t('hingedSystem.basic') });
...
HingedSystem.defaultProps = {
  orderID: null,
};

HingedSystem.propTypes = {
  toastManager: PropTypes.shape(notificationPropTypes).isRequired,
  orderID: PropTypes.string,
};

export default withToastManager(HingedSystem);
